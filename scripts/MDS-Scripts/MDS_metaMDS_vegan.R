...
##### RUNNING THE MDS ######
  ...
  ##Running metaMDS from vegan package 
  # Assuming dissimilarity_matrix is your dissimilarity matrix
  # Set the number of dimensions
  ndims <- 7
  
  # Run metaMDS
  mds_result <- metaMDS(dissimilarity_matrix, k = ndims)
...
##Scree Plot METAMDS
  ...
  # Initialize an empty vector to store stress values
  stress_values <- c()
  
  # Number of dimensions to evaluate
  max_dims <- 23  # You can change this to another value
  
  # Loop through and run metaMDS for 1 to max_dims dimensions
  for (k in 1:max_dims) {
    mds_result_k <- metaMDS(dissimilarity_matrix, k = k)
    stress_values[k] <- mds_result_k$stress
  }
  
  # Create a scree plot of stress values
  plot(1:max_dims, stress_values, type = "b", 
       xlab = "Number of Dimensions", 
       ylab = "Stress Value", 
       main = "Scree Plot of Stress Values")



##2D plots for metaMDS

  # Number of dimensions in metaMDS
  max_dims = dim(mds_result$points)[2]  # This will dynamically set max_dims based on your result
  ...
  for(i in 1:(max_dims - 1)) {
    for(j in (i + 1):max_dims) {
      
      # Calculate axis limits
      xlims = range(mds_coordinates[, i]) + c(-0.1, 0.1)  # "Zoom out" by 0.1 unit
      ylims = range(mds_coordinates[, j]) + c(-0.1, 0.1)  # "Zoom out" by 0.1 unit
